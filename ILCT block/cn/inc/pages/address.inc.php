...
<div class='row-fluid'>
  <div class='span4'>
	<table class='table table-striped table-condensed'>
      <tr><td><b>交易地址:</b></td><td><?php echo $ainfo[0]['address']; ?></td></tr>
      <tr><td><b>交易数量:</b></td><td><?php echo "$clean_bal $curr_code $conf_txt"; ?></td></tr>
      <tr><td><b>发送总数:</b></td><td><?php echo "$real_ins $curr_code"; ?></td></tr>
      <tr><td><b>接收总数:</b></td><td><?php echo "$real_ots $curr_code"; ?></td></tr>
      <tr><td><b>最后交易:</b></td><td><?php echo $last_used; ?></td></tr>
	</table>
  </div>
  <div class='span4'>
    <table class='table table-striped table-condensed'>
      <tr><td><b>交易次数:</b></td><td><?php echo $real_txc; ?></td></tr>
      <tr><td><b>发送次数:</b></td><td><?php echo $real_inc; ?></td></tr>
	  <tr><td><b>接收次数:</b></td><td><?php echo $real_otc; ?></td></tr>
      <tr><td><b>交易限额:</b></td><td><?php echo "$clean_lim $curr_code"; ?></td></tr>
      <tr><td><b>待定交易:</b></td><td><?php echo "$clean_fli $curr_code"; ?></td></tr>
	</table>
  </div>
  <div class='span4'>
    <center>
...
<form id="txlist_form" method="get" action="./">
  ...
  <select name='filter' id="filter_select">
	...
	<option value="0"<?php echo $sel_txt; ?>>全部</option>
	<?php $sel_txt = ($filter == 1) ? ' selected="selected"' : ''; ?>
	<option value="1"<?php echo $sel_txt; ?>>发送</option>
	<?php $sel_txt = ($filter == 2) ? ' selected="selected"' : ''; ?>
	<option value="2"<?php echo $sel_txt; ?>>接收</option>
	<?php $sel_txt = ($filter == 3) ? ' selected="selected"' : ''; ?>
	<option value="3"<?php echo $sel_txt; ?>>例外</option>
  </select>
  <select name='sort' id="sort_select">
	<?php $sel_txt = ($sort_meth == 0) ? ' selected="selected"' : ''; ?>
	<option value="0"<?php echo $sel_txt; ?>>最新区块</option>
	<?php $sel_txt = ($sort_meth == 1) ? ' selected="selected"' : ''; ?>
	<option value="1"<?php echo $sel_txt; ?>>原始区块</option>
  </select>
</form>
...
<?php
  ...
  if (empty($tx_set)) {
	...
	foreach ($tx_set as $tx_key => $tx) {
	  ...
	  $tx_time = empty($tx['time']) ? date("Y-m-d h:i:s A") : date("Y-m-d h:i:s A", $tx['time']);
	  ...
	  foreach ($tx['vout'] as $key => $value) {
		...
		if (isset($tx['limit'])) {
		  echo "输入地址提取限制设置为: <span class='happy_txt'>".
